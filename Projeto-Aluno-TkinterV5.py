...
def corrigir_provas(prova_arquivo):
    ...
    global dados_prova, respostas_aluno_corrigir, numero_questao_corrigir, nome_aluno_corrigir, aluno_index
    ...
    try:
        ...
        messagebox.showerror("Erro", f"Erro ao carregar a prova ou respostas: {e}")
        return
...
def mostrar_questao_correcao(aluno_index):
    global numero_questao_corrigir, dados_prova, respostas_aluno_corrigir, nome_aluno_corrigir
    ...
    if questao["tipo"] == "Objetiva":
        ...
        # Nota automática baseada na resposta
        nota_auto = 1.0 if resposta_aluno == gabarito else 0.0
        ...
        entrada_nota.insert(0, str(nota_auto))
        entrada_nota.pack(pady=5)

        # Salvar essa entrada em variável global ou estrutura para usar depois na correção final

    else:
        resposta_texto = tk.Text(frame_correcao_provas, height=8, width=80, font=("Arial", 12))
        resposta_texto.insert("1.0", resposta_aluno)
        resposta_texto.config(state="disabled")  # não editável
    ...
    def ir_proximo_aluno():
        ...
        if aluno_index + 1 < len(respostas_aluno_corrigir):
            numero_questao_corrigir = 0
            mostrar_questao_correcao(aluno_index + 1)
        else:
